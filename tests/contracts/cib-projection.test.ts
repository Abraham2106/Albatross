...
function draft(overrides: Partial<VisitDraft> = {}): VisitDraft {
  ...
  return {
    id: 'd1', site: createSite({ id: 's1', name: 'Hospital DemoCare Green', city: 'San Jose', country: 'Costa Rica' }),
...
describe('CIB contract projection', () => {
  it('maps a site to the hospital list and ficha', () => {
    ...
    expect(cliente).toMatchObject({ id: 's1', nombre: 'Hospital DemoCare Green', ciudad: 'San Jose', pais: 'Costa Rica' });
  ...
  it('turns a draft into confirmation cards and applies si/no/nose', () => {
    ...
    expect(obs).toMatchObject({ ciudad: 'San Jose', pais: 'Costa Rica', confianza: null });
  ...
  it('resolves a complete mention to an existing site', () => {
    const sites = [createSite({ id: 's1', name: 'Hospital DemoCare Green', city: 'San Jose', country: 'Costa Rica' })];
    const resolved = resolveHospital({ name: 'hospital democare green', city: 'San Jose', country: 'Costa Rica', evidence: 'x' }, sites);
